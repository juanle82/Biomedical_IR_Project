...
try:
    ...
    logger.error("Error: could not find libuvc!")
    exit(1)
...
def print_device_info(devh):
    ...
    logger.info("FLIR part #: {0}".format(flir_pn.raw))

    flir_sn = create_string_buffer(8)
    call_extension_unit(devh, SYS_UNIT_ID, 3, flir_sn, 8)
    logger.info("FLIR serial #: {0}".format(repr(flir_sn.raw)))
...
def print_device_formats(devh):
    for format_desc in uvc_iter_formats(devh):
        logger.info("format: {0}".format(format_desc.guidFormat[0:4]))
...
def set_manual_ffc(devh):
    ...
    logger.debug("controlID: " + str(controlID))
